...
func New(
	...
	// Initialize snapshot accounts map
	snapAccounts := make(map[uint16]map[uint16]int32)
	...
	for balance := range bankRetChan {
		...
		snapAccounts[balance.BankId] = make(map[uint16]int32)
	}
